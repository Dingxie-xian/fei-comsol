...
public class step1_v2_piston_pml {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Model model = ModelUtil.create("step1_v2_piston_pml");
        ...
        model.param().set("Z_down", "330[mm]");    // extended (Z_up + PML 20mm = 310 + PML 330)
        ...
        // Air main: 0 < r < R_domain, -Z_up < z < Z_down-20 (i.e. ends at z=310)
        ...
        // PML top: 0 < r < R_domain, Z_down-20 < z < Z_down (i.e. z ∈ [310, 330])
        ...
        // PML right: R_domain < r < R_domain+20, -Z_up < z < Z_down-20
        ...
        // PML corner: R_domain < r < R_domain+20, Z_down-20 < z < Z_down
        ...
        // Union the three PML rectangles into single PML domain
        ...
        log("Geometry: air " + 150 + "x" + (300+310) + "mm + PML top/right/corner 20mm");
        ...
        // Apply PML coordSystem to the PML union domain
        // Identify PML union domain ID from geometry
        int pml_domain_id = -1;
        try {
            // probe the PML union domain — it's a domain, not a boundary
            Object selList = comp.getClass().getMethod("selection").invoke(comp);
            ...
            if (doms != null && doms.length > 0) {
                pml_domain_id = doms[0];
                log("Probed PML union domain id = " + pml_domain_id);
            }
        } catch (Exception e) { log("PML probe failed: " + e.getMessage()); }

        if (pml_domain_id > 0) {
            try {
                // PML with Cylindrical scaling for axisymmetric geometry
            ...
            } catch (Exception e) { log("FAIL pml1: " + e.getMessage()); allOk = false; }
        } else {
            log("!!! PML domain not identified, skipping coordSystem. Step3 simulation will still run but PML inactive.");
        }
        ...
        // For PML: no top/right PWR. Instead we leave the air-PML interface free (no BC).
        // The PML outer edges (z=330, r=170, corner) need a BC: Sound Hard.
        ...
        if (idSrc < 0 || idBaffle < 0 || idPMLouterTop < 0 || idPMLouterRight < 0) {
        ...
        // 3. PML outer edges (z=330, r=170, corner): Sound Hard
        // Air-PML interface has NO BC; PML coordSystem stretches coordinates there.
        try {
            comp.physics("acpr").create("pmlt_sh", "SoundHard", 1);
            comp.physics("acpr").feature("pmlt_sh").selection().set(idPMLouterTop);
            log("OK pmlt_sh: SoundHard on id=" + idPMLouterTop);
        } catch (Exception e) { log("FAIL pmlt_sh: " + e.getMessage()); allOk = false; }

        try {
            comp.physics("acpr").create("pmlr_sh", "SoundHard", 1);
            comp.physics("acpr").feature("pmlr_sh").selection().set(idPMLouterRight);
            log("OK pmlr_sh: SoundHard on id=" + idPMLouterRight);
        } catch (Exception e) { log("FAIL pmlr_sh: " + e.getMessage()); allOk = false; }

        try {
            comp.physics("acpr").create("pmlc_sh", "SoundHard", 1);
            comp.physics("acpr").feature("pmlc_sh").selection().set(idPMLouterCorner);
            log("OK pmlc_sh: SoundHard on id=" + idPMLouterCorner);
        } catch (Exception e) { log("FAIL pmlc_sh: " + e.getMessage()); allOk = false; }
        ...
        // Do NOT abort on Continuity-not-bindable. It's a documented COMSOL API limit.
        // The default Continuity is physically correct (same fluid both sides).
