...
/**
 * Step 2 of FZP simulation: piston source + Z_down extended to 300 mm.
 *
 * Differences vs step1 (full-aperture plane wave):
 * - a_trans = 8 mm (piston source, not 150)
 * - Source: Normal Velocity on r ∈ [0, 8] only (NOT full bottom)
 * - Baffle: Sound Hard on r ∈ [8, 150] of bottom (RESTORED)
 * - Z_down = 300 mm (extended from 200, since piston source has shifted focal point ~200 mm)
 * - Point split restored at (a_trans, -Z_up) = (8, -300) to divide bottom edge
 *
 * Goal: verify piston source produces focus, target z_max ∈ [190, 210] mm.
 */
...
public class step2 {
    ...
    public static void main(String[] args) throws Exception {
        log("=== step2: piston source, Z_up=300, Z_down=300 ===");
        ...
        model.param().set("a_trans", "8[mm]");  // step2: piston source (TCT40-16T)
        model.param().set("Z_down", "350[mm]");  // v2: extend to capture shifted focus at ~284mm
        ...
        // pt_src at (8, -300): splits bottom edge into [0,8] source + [8,150] baffle
        ...
        // step2: piston source, Point at (8,-300) splits bottom edge.
        ...
        try {
            ...
            int[] bEnts = comp.physics("acpr").feature("baffle").selection().entities();
            log("OK baffle: SoundHard on id=" + idBaffle + " (entities=" + java.util.Arrays.toString(bEnts) + ")");
        } catch (Exception e) {
        ...
        // Mesh: global lam/6 + local refinement z ∈ [200, 350] with lam/8 for shifted focus
        ...
        // Use Box selection for refinement region
